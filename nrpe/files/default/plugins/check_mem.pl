...
use vars qw($opt_c $opt_f $opt_u $opt_w
	    ...
	    $crit_level $warn_level
            %exit_codes @memlist
            $percent $fmt_pct 
            $verb_err $command_line $performance_data);
...
if ($opt_f)
{
  ...
  $performance_data = "|mem_free=$percent;$warn_level;$crit_level;0"; 
  if ($percent <= $crit_level)
  {
    print "Memory CRITICAL - $fmt_pct% ($free_memory kB) free$performance_data\n";
    exit $exit_codes{'CRITICAL'};
  }
  elsif ($percent <= $warn_level)
  {
    print "Memory WARNING - $fmt_pct% ($free_memory kB) free$performance_data\n";
    exit $exit_codes{'WARNING'};
  }
  else
  {
    print "Memory OK - $fmt_pct% ($free_memory kB) free$performance_data\n";
    exit $exit_codes{'OK'};
  }
}
elsif ($opt_u)
{
  ...
  $fmt_pct    = sprintf "%.1f", $percent;
  $performance_data = "|mem_use=$percent;$warn_level;$crit_level;0"; 
  if ($percent >= $crit_level)
  {
    print "Memory CRITICAL - $fmt_pct% ($used_memory kB) used$performance_data\n";
    exit $exit_codes{'CRITICAL'};
  }
  elsif ($percent >= $warn_level)
  {
    print "Memory WARNING - $fmt_pct% ($used_memory kB) used$performance_data\n";
    exit $exit_codes{'WARNING'};
  }
  else
  {
    print "Memory OK - $fmt_pct% ($used_memory kB) used$performance_data\n";
    exit $exit_codes{'OK'};
  }
}
